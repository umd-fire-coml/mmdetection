...
class CascadeRCNN(BaseDetector, RPNTestMixin):
    ...
    def forward_train(self,
        ...
        if self.with_rpn:
            ...
            proposal_list = self.rpn_head.get_bboxes(*proposal_inputs) # possible filters here
        else:
        ...
        for i in range(self.num_stages):
            ...
            if self.with_bbox or self.with_mask:
                ...
                for j in range(num_imgs):
                    assign_result = bbox_assigner.assign( # possible filters here
                        proposal_list[j], gt_bboxes[j], gt_bboxes_ignore[j],
                        gt_labels[j])
                    sampling_result = bbox_sampler.sample( # possible filters here
                        assign_result,
                        proposal_list[j],
                        gt_bboxes[j],
                        gt_labels[j],
            ...
            bbox_feats = bbox_roi_extractor(x[:bbox_roi_extractor.num_inputs], # possible conv is here
                                            rois)
            if self.with_shared_head:
